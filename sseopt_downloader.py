...
cfg = ConfigParser()
cfg.read('config.ini')
opt_file_paths = cfg.get('path', 'optdata_save_path').split(',')
...
def send_sseopt_file():
    ...
    msg['To'] = _format_addr('余君 <yujun_mail@163.com, yujun_mail@sian.com>')
    ...
    # 添加附件（即加上一个MIMEBase）
    attached_file_name = os.path.join(opt_file_paths[0], 'SSEOptContract%s.txt' % datetime.date.today().strftime('%Y%m%d'))
    ...
    server.login('yujun_mail@163.com', 'm8LpEqXkWR7mAY3')
...
for opt_file_path in opt_file_paths:
